# NW.js macOS Build Fix

## Issue
macOS builds were failing with the following error:
```
...
### What is LSApplicationCategoryType?
...
### Why nw-builder Requires It
When building for macOS, nw-builder validates that this property is set to ensure proper macOS app bundle creation. The validation happens during the build process, before the actual packaging.
...
nw-builder couldn't read it properly when using `package.json` as the source file. This is because:
1. nw-builder reads package.json for build configuration
2. The validation expects the value to be accessible via `options.app.LSApplicationCategoryType`
3. Simply having it at the root level of package.json wasn't sufficient for nw-builder's validation

## Solution

### Approach
Pass `LSApplicationCategoryType` as a command-line argument specifically for macOS builds.
...
- name: Build NW.js for ${{ matrix.platform }}
  ...
  run: |
    ...
    # For macOS, add LSApplicationCategoryType argument
...
2. **Direct injection**: Provides the value exactly where nw-builder's validation expects it
...
### Before Fix
- ❌ macOS builds failed immediately during validation
- ❌ No .app bundle created
- ❌ No artifacts uploaded
- ❌ CI workflow failed

### After Fix
- ✅ macOS builds pass validation
...
- **Commit:** 38d1299
- **Date:** 2026-02-10
- **Message:** Fix macOS build - pass LSApplicationCategoryType as nw-builder argument
